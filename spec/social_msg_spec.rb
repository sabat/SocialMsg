...
describe SocialMsg do
  ...
  context "an instance" do
    ...
    it "can print its text without a URL" do
      ...
      expect( social_msg.to_s ).to_not match %r{ http:// }
    end
    ...
    it "does not add a colon to the name if the name is empty" do
      social_msg.name = ''
      expect( social_msg.to_text ).to_not match /^:/
    end

    it "does not print extra spaces if a field is empty" do
      social_msg.name = ''
      social_msg.title = ''
      expect( social_msg.to_text ).to_not match /\s\s/
    end
    ...
    it "can reset itself to its original state" do
      ...
      expect { social_msg.shorten! }.to change { social_msg.to_text }
    ...
    it "can do all modifications (hashtag, short URL, shorten text) with one method" do
      ...
      expect { social_msg.shorten! }.to change { social_msg.to_text }
      social_msg.to_text.should_not be_empty
      social_msg.to_text.should have_at_most(140).characters
    end
  ...
  context "doing hashtagging" do
    ...
    it "does not add hashtags if the keywords obj is invalid" do
      SocialMsg.hashtag_words = []
      expect { social_msg.hashtag!(900) }.to_not change { social_msg.to_text }
    end
  
    it "does not add a hashtag if a keyword already has one" do
      SocialMsg.hashtag_words = words
      social_msg.title = '#Bigfoot #UFO sightings in the same place!'
      expect { social_msg.hashtag! }.to_not change { social_msg.to_text }
    end
  end
  ...
  context "when shortening the text" do
    ...
    it "can trim the title if the max len is passed in" do
      expect { social_msg.trimmed_title!(140) }.to change { social_msg.to_text }
      social_msg.to_text.should_not be_empty
      social_msg.to_text.should have_at_most(140).characters
    end
    ...
    it "can trim the title if the max len is set at the class level" do
      ...
      expect { social_msg.trimmed_title! }.to change { social_msg.to_text }
      social_msg.to_text.should_not be_empty
      social_msg.to_text.should have_at_most(110).characters
    end
    ...
    it "can trim the title after having hashtags and URL shortening" do
      ...
      expect { social_msg.hashtag!.short_url!.trimmed_title! }.to change { social_msg.to_text }
      social_msg.to_text.should_not be_empty
      social_msg.to_text.should have_at_most(140).characters
    end
  end
